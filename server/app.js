...
app.get("/caretaker/all", (req, res) => {
    ...
    db.handleQuery(connectionPool, {
        ...
        values: [req.query.userID, maxPerPagination, currentPaginationOffset]
    }, (data) => {
        res.status(httpOkCode).json(data);
    }, (err) => res.status(badRequestCode).json({reason: err}))
})
...
app.get("/caretaker/all/count", (req, res) => {
    ...
    db.handleQuery(connectionPool, {
        query: "SELECT Count(*) FROM `rehabilitator` as `r` INNER JOIN `caretaker` as `c` on `r`.`caretaker_id` = `c`.`caretaker_id` INNER JOIN `user` as `u` on `u`.`id` = `c`.`user_id` WHERE `u`.`id` = ?",
